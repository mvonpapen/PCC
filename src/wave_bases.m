function [daughter,fourier_factor,coi,dofmin] = wave_bases(mother,k,scale,param);
	...
	% 	daughter = daughter.*(k > 0.);     % Heaviside step function
	    % New normalization because of problems close to Nyquist frequency
	    % power close to Nyquist was observed to fall off drastically
	    daughter = exp(expnt); % new normalization by Mitch, 23.05.16
	    daughter = daughter / sqrt(sum(abs(daughter).^2)/n); % new normalization by Mitch, 23.05.16
